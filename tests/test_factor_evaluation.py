...
def test_signal_evaluation_reuses_prepared_prices_and_scheduled_returns() -> None:
    ...
    scheduled = prediction_forward_returns(
        signals.select(
    ...
    direct = run_prediction_evaluation(
        _scheduled_signal(signals),
        prices,
        config=BacktestConfig(initial_capital=10_000, quantiles=2, top_n=1),
    )
    reused = run_prediction_evaluation(
        _scheduled_signal(signals),
        prices,
        config=BacktestConfig(initial_capital=10_000, quantiles=2, top_n=1),
...
def test_prediction_evaluation_without_synthetic_diagnostics_runs_primary_path(
    ...
    assert result.top_n_backtest.returns.height > 0
    assert result.quantile_returns.is_empty()
...
def test_signal_diagnostics_build_only_requested_paths(
    ...
    quantiles = materialize_prediction_diagnostics(
        _scheduled_signal(signals),
        prices,
        config=BacktestConfig(initial_capital=10_000, quantiles=2, top_n=1),
    ...
    spread = materialize_prediction_diagnostics(
        _scheduled_signal(signals),
        prices,
        config=BacktestConfig(initial_capital=10_000, quantiles=2, top_n=1),
        include_spread=True,
    ...
    combined = materialize_prediction_diagnostics(
        _scheduled_signal(signals),
        prices,
        config=BacktestConfig(initial_capital=10_000, quantiles=2, top_n=1),
        include_quantiles=True,
...
def test_signal_diagnostics_skip_unrequested_family_builders(
    ...
    lags = materialize_prediction_diagnostics(
        _scheduled_signal(signals),
        prices,
        config=config,
        include_lags=True,
...
def test_batched_lag_bankruptcy_is_isolated_per_portfolio() -> None:
    ...
    diagnostics = materialize_prediction_diagnostics(
        _scheduled_signal(signals),
        prices,
        config=BacktestConfig(
            initial_capital=10,
